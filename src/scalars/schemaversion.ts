...
export const SchemaVersionScalar = new GraphQLScalarType({
  ...
  serialize (value: unknown) {
    return (value as DateTime).toUTC().toFormat(schemaVersionFormat)
  },

  parseValue (value: unknown) {
    const parsedValue = DateTime.fromFormat(value as string, schemaVersionFormat, { zone: 'utc' })
  ...
  parseLiteral (ast: ValueNode) {
    if (ast.kind === Kind.STRING) {
      const parsedValue = DateTime.fromFormat(ast.value, schemaVersionFormat, { zone: 'utc' })
      if (!parsedValue.isValid) throw new Error(`Invalid date: ${parsedValue.invalidReason!} ${parsedValue.invalidExplanation!}`)
      return parsedValue
    }
    return null
