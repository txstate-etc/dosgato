...
export async function createPageRoutes (app: FastifyInstance) {
  ...
  app.post<{ Params: { parentPageId: string }, Body?: CreatePageInput }>('/pages/:parentPageId', async (req, res) => {
    ...
    logMutation(new Date().getTime() - startTime.getTime(), 'importPage', 'mutation uploadImportPage (RESTful)', ctx.auth, { parentId: parent.id }, { success: true, pagesImported: Object.values(parentsByPath).map(p => p.id) }, [], ctx).catch(console.error)
  ...
  app.post<{ Params: { parentPageId: string }, Body?: CreatePageInput }>('/pages/migrate/:parentPageId', async (req, res) => {
    ...
    logMutation(new Date().getTime() - startTime.getTime(), 'importPage', 'mutation uploadMigratePage (RESTful)', ctx.auth, { parentId: parent.id, name: newPageName }, { success: true, id: page.id }, [], ctx).catch(console.error)
