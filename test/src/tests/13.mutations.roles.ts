...
describe('roles mutations', () => {
  ...
  it('should not allow an unauthorized user to create a role', async () => {
    await expect(queryAs('ed07', 'mutation CreateRole ($name: String!) { createRole (name: $name) { success role { id name } } }', { name: 'doesnotmatter' })).to.be.rejected
  })
  ...
  it('should not allow an unauthorized user to update a role name', async () => {
    const { createRole: { role: roleBB } } = await query('mutation CreateRole ($name: String!) { createRole (name: $name) { success role { id name } } }', { name: 'roleBB' })
  ...
  it('should not allow an unauthorized user to delete a role', async () => {
    const { createRole: { role: roleDD } } = await query('mutation CreateRole ($name: String!) { createRole (name: $name) { success role { id name } } }', { name: 'roleDD' })
  ...
  it('should not allow an unauthorized user to assign a role to a user', async () => {
    const { createRole: { role: roleEE } } = await query('mutation CreateRole ($name: String!) { createRole (name: $name) { success role { id name } } }', { name: 'roleEE' })
