...
describe('users mutations', () => {
  ...
  it('should disable a user', async () => {
    const { disableUsers: { success } } = await query('mutation DisableUsers ($id: [String]!) { disableUsers(userIds: $id) { success users { id name } } }', { id: ['ed10'] })
    expect(success).to.be.true
  ...
  it('should not disable a non-existent user', async () => {
    await expect(query('mutation DisableUsers ($id: [String]!) { disableUsers(userIds: $id) { success users { id name } } }', { id: ['fakeuser'] })).to.be.rejected
  ...
  it('should not allow an unauthorized user to disable a user', async () => {
    await expect(queryAs('ed07', 'mutation DisableUsers ($id: [String]!) { disableUsers(userIds: $id) { success users { id name } } }', { id: ['su01'] })).to.be.rejected
  })
})
