...
describe('users mutations', () => {
  ...
  it('should not update a non-existent user', async () => {
    await expect(query('mutation UpdateUser ($id: ID!, $input: UpdateUserInput!) { updateUser (userId: $id, args: $input) { success user { id name } } }', { id: 'notreal', input: { name: 'Should Notwork' } })).to.be.rejected
  ...
  it('should not allow an unauthorized user to update a user', async () => {
    await expect(queryAs('ed07', 'mutation UpdateUser ($id: ID!, $input: UpdateUserInput!) { updateUser (userId: $id, args: $input) { success user { id name } } }', { id: 'ed10', input: { name: 'Updated Username' } })).to.be.rejected
  })
  it('should disable a user', async () => {
    const { disableUsers: { success } } = await query('mutation DisableUsers ($ids: [ID!]!) { disableUsers(userIds: $ids) { success users { id name } } }', { ids: ['ed10'] })
    expect(success).to.be.true
  ...
  it('should not disable a non-existent user', async () => {
    await expect(query('mutation DisableUsers ($ids: [ID!]!) { disableUsers(userIds: $ids) { success users { id name } } }', { ids: ['fakeuser'] })).to.be.rejected
