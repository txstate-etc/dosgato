...
describe('data mutations', () => {
  ...
  it('should move multiple data entries within a data folder', async () => {
    ...
    const { data: data1 } = await createDataEntry('Snow', 'keyd1', { title: 'Snow Text', color: 'snow', align: 'left' }, undefined, folderQ.id)
    const { data: data2 } = await createDataEntry('Light Pink', 'keyd1', { title: 'Light Pink Text', color: 'lightpink', align: 'left' }, undefined, folderQ.id)
    const { data: data3 } = await createDataEntry('Dark Pink', 'keyd1', { title: 'Dark Pink Text', color: 'darkpink', align: 'left' }, undefined, folderQ.id)
  ...
  it('should move multiple data entries out of a folder to a site', async () => {
    ...
    const { data: data1 } = await createDataEntry('Amaranth', 'keyd1', { title: 'Amaranth Text', color: 'amaranth', align: 'left' }, undefined, folderR.id)
    const { data: data2 } = await createDataEntry('Amber', 'keyd1', { title: 'Amber Text', color: 'amber', align: 'left' }, undefined, folderR.id)
    const { data: data3 } = await createDataEntry('Amethyst', 'keyd1', { title: 'Amethyst Text', color: 'amethyst', align: 'left' }, undefined, folderR.id)
  ...
  it('should move multiple data entries out of a folder to global data', async () => {
    ...
    const { data: data1 } = await createDataEntry('Aquamarine', 'keyd1', { title: 'Aquamarine Text', color: 'aquamarine', align: 'left' }, undefined, folder.id)
    const { data: data2 } = await createDataEntry('Azure', 'keyd1', { title: 'Azure Text', color: 'azure', align: 'left' }, undefined, folder.id)
    const { data: data3 } = await createDataEntry('Baby-Blue', 'keyd1', { title: 'Baby Blue Text', color: 'babyblue', align: 'right' }, undefined, folder.id)
    const { data: data4 } = await createDataEntry('Blue-Green', 'keyd1', { title: 'Blue-Green Text', color: 'blue-green', align: 'left' }, undefined, folder.id)
  ...
  it('should move multiple site-level data entries within a site', async () => {
    ...
    const { data: data1 } = await createDataEntry('Blush', 'keyd1', { title: 'Blush Text', color: 'blush', align: 'left' }, site.id)
    const { data: data2 } = await createDataEntry('Cerise', 'keyd1', { title: 'Cerise', color: 'cerise', align: 'left' }, site.id)
    const { data: data3 } = await createDataEntry('Cerulean', 'keyd1', { title: 'Cerulean Text', color: 'cerulean', align: 'left' }, site.id)
    const { data: data4 } = await createDataEntry('Champagne', 'keyd1', { title: 'Champagne Text', color: 'champagne', align: 'left' }, site.id)
  ...
  it('should move multiple site-level data entries to a folder', async () => {
    ...
    const { data: data1 } = await createDataEntry('Khaki', 'keyd1', { title: 'Khaki Text', color: 'khaki', align: 'right' }, site.id)
    const { data: data2 } = await createDataEntry('Ivory', 'keyd1', { title: 'Ivory Text', color: 'ivory', align: 'left' }, site.id)
    const { data: data3 } = await createDataEntry('Goldenrod', 'keyd1', { title: 'GoldenRod Text', color: 'goldenrod', align: 'left' }, site.id)
    const { data: data4 } = await createDataEntry('Tan', 'keyd1', { title: 'Tan Text', color: 'tan', align: 'center' }, site.id)
    const { data: data5 } = await createDataEntry('Salmon', 'keyd1', { title: 'Salmon Text', color: 'salmon', align: 'center' }, site.id)
  ...
  it('should move multiple site-level data entries to global data', async () => {
    ...
    const { data: data1 } = await createDataEntry('Peach', 'keyd1', { title: 'Peach Text', color: 'peach', align: 'left' }, site.id)
    const { data: data2 } = await createDataEntry('Coral', 'keyd1', { title: 'Coral Text', color: 'coral', align: 'left' }, site.id)
    const { data: data3 } = await createDataEntry('Rosewood', 'keyd1', { title: 'Rosewood Text', color: 'rosewood', align: 'left' }, site.id)
    const { data: data4 } = await createDataEntry('Ruby', 'keyd1', { title: 'Ruby Text', color: 'ruby', align: 'left' }, site.id)
    const { data: data5 } = await createDataEntry('Crimson', 'keyd1', { title: 'Crimson Text', color: 'crimson', align: 'center' }, site.id)
    const { success } = await moveDataEntries([data2.id, data3.id])
    expect(success).to.be.true
  ...
  it('should update the display order of global data entries when multiple entries are moved', async () => {
    const { data: data1 } = await createDataEntry('Plum', 'keyd1', { title: 'Plum Text', color: 'plum', align: 'left' })
    const { data: data2 } = await createDataEntry('Eggplant', 'keyd1', { title: 'Eggplant Text', color: 'eggplant', align: 'left' })
    const { data: data3 } = await createDataEntry('Blueviolet', 'keyd1', { title: 'Blueviolet Text', color: 'blueviolet', align: 'left' })
    const { data: data4 } = await createDataEntry('Steelblue', 'keyd1', { title: 'Steelblue Text', color: 'steelblue', align: 'left' })
  ...
  it('should move multiple global data entries to a folder', async () => {
    const { data: data1 } = await createDataEntry('Turquoise', 'keyd1', { title: 'Turquoise Text', color: 'turquoise', align: 'left' })
    const { data: data2 } = await createDataEntry('Seagreen', 'keyd1', { title: 'Seagreen Text', color: 'coral', align: 'left' })
    const { data: data3 } = await createDataEntry('Emerald', 'keyd1', { title: 'Emerald Text', color: 'emerald', align: 'left' })
    const { data: data4 } = await createDataEntry('Jade', 'keyd1', { title: 'Jade Text', color: 'jade', align: 'left' })
    const { data: data5 } = await createDataEntry('Celadon', 'keyd1', { title: 'Celadon Text', color: 'celadon', align: 'center' })
  ...
  it('should move multiple global data entries to a site', async () => {
    ...
    const { data: data1 } = await createDataEntry('Sage', 'keyd1', { title: 'Sage Text', color: 'sage', align: 'left' })
    const { data: data2 } = await createDataEntry('Taupe', 'keyd1', { title: 'Taupe Text', color: 'taupe', align: 'left' })
    const { data: data3 } = await createDataEntry('Slate', 'keyd1', { title: 'Slate Text', color: 'slate', align: 'left' })
    const { data: data4 } = await createDataEntry('Eggshell', 'keyd1', { title: 'Eggshell Text', color: 'eggshell', align: 'left' })
    const { data: data5 } = await createDataEntry('Applegreen', 'keyd1', { title: 'Applegreen Text', color: 'applegreen', align: 'center' })
  ...
  it('should move data entries from different locations', async () => {
    ...
    const { data: data1 } = await createDataEntry('Orchid', 'keyd1', { title: 'Orchid Text', color: 'orchid', align: 'left' }, undefined, folderV.id)
    const { data: data2 } = await createDataEntry('RoyalBlue', 'keyd1', { title: 'Royal Blue Text', color: 'royalblue', align: 'left' }, undefined, folderV.id)
    const { data: data3 } = await createDataEntry('Merangue', 'keyd1', { title: 'Merangue Text', color: 'merangue', align: 'left' }, undefined, folderV.id)
    const { data: data4 } = await createDataEntry('Midnight', 'keyd1', { title: 'Midnight Text', color: 'midnight', align: 'left' }, undefined, folderW.id)
    const { data: data5 } = await createDataEntry('Linen', 'keyd1', { title: 'Linen Text', color: 'linen', align: 'center' }, undefined, folderW.id)
    const { data: data6 } = await createDataEntry('MistyRose', 'keyd1', { title: 'Misty Rose Text', color: 'mistyrose', align: 'center' }, undefined, folderW.id)
